...
from tkinter import *
bg = PhotoImage(file="vo.png")
label1 = Label(root, image=bg)
label1.place(x=0, y=0)

f=PhotoImage(file="QR.png")
# Create a button with the image and text
Button(root, text="Tutorial", image=f,compound=BOTTOM).place(x=1480,y=800)
x=0
a=0
n=0
t=[]
...
def verify():
    ...
    username_label = p.Label(frame, text="Username:")
    username_label.grid(row=0, column=0, sticky="w", padx=5, pady=5)

    username_entry = p.Entry(frame)
    username_entry.grid(row=0, column=1, padx=5, pady=5)

    password_label = p.Label(frame, text="Password:")
    password_label.grid(row=1, column=0, sticky="w", padx=5, pady=5)

    password_entry = p.Entry(frame, show="*")
    password_entry.grid(row=1, column=1, padx=5, pady=5)

    submit_button = p.Button(frame, text="Submit", command=verify_user)
...
btn=tk.CTkButton(master=buttonframe2,text="Delete Voting Logs from temp",command=verify,font=("MV Boli", 18))
btn.grid(row=2,column=0,sticky=tk.W+tk.E)
